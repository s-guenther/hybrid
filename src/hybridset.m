...
%
%   verbose - Verbosity level of toolbox. Sets the amount of information
%     the user will be provided with during the calculation. Can be 0, 1 or
%     2.  Default is 0, which means no info messages appear and the
%     calculation runs silently. Higher values result in more info
%     messages.
%
%   discrete_solver - Solver which is used if the signal is of discrete
%     type 'step' or 'linear'. Can be 'exact' or 'time_preserving'. Default
%     is 'exact', which inserts extra timesteps into original vector value
%     pair if neccessary (If a storage runs full/empty in between a
%     timestep). Option 'time_preserving' does this not.
%   continuous_solver - ode solver which is used for continuous signal
%     'fhandle'. Must be a function handle to an existing Matlab ODE
%     solver.  Default is @ode45. A good alternative to try is @ode23t.
...
%     parameters).
%
%   amv_rel_tol - Average mean value relative tolerance. Default is 1e-3.
%     The input signal has to have an AMV of zero. amv_rel_tol defines the
%     allowed relative deviation from this requirement.
%   int_neg_rel_tol - Negative integral relative tolerance. Default is
%     1e-3. The integral of the input signal is not allowed to become
%     negative within the whole period. This defines the allowed temporary
%     violation of this requirement.
%   int_zero_rel_tol - Zero integral relative tolerance. Default is 1e-3.
%     The integral of the signal must be zero at the end of the period to
%     ensure real periodicity. This is the allowed violation of this
%     requirement.
%   dismissed_power_rel_tol - Dismissed power relative tolerance. Default
%     is 1e-3. If the control for a specific case is simulated to view or
%     verify a dimensioning result in operation, the input power must
%     always match the power of the storages. Due to numeric errors, this
%     may fail at some points in time. This value defines the integral
%     error which is allowed to occur to still accept the result as valid.
%   ampl_sample - Sampling points for finding the maximum amplitute, as
%     finding the maximum with fminbnd may fail. Default 1e5
%   state_tol - Tolerance for control state detection. Default is 1e-2.
%   
...
%     type is 'fhandle'. The backward integral is obtained through
%     numerical integration, to subsequently gain a function handle again,
%     this result is interpolated. Default is 'pchip'. Can be 'linear',
%     'pchip' or 'spline'.
%   intersim - Interpolation method for the simulation results if signal
%     type is 'fhandle'. Default is 'spline', can be 'linear', 'spline' or
%     'pchip'. The simulation results are obtained through numerical
%     integration, to subsequently gain a function handle again, this
%     result is interpolated. 
%   interint - Interpolation method for signal integral if signal type is
%     'fhandle'. Default is 'spline', can be 'linear' or 'pchip'.
%
%   tanh_sim - Hyperbolic tangent gradient. For Smoothing of requested
%     power of peak storage in control strategy. For numeric stability,
%     only relevant for continuous signal ('fhandle'). Higher values lead
%     to more accurate results by increasing the instability, default is
%     1e3
%   tanh_dim - Hyperbolic tangent gradient. This options allows a smoothing
%     of discontinuities within an input signal of type 'fhandle'. Note
%     that this discontinuities emerge inevitably within the power
%     splitting procedure independent of the input function. This can slow
%     down ode integration significantly and a tanh approximation at
%     discontinuities may help to resolve this issue. Use this option only
%     if the original signal does not show discontinuities, else try the
%     conv option. Default is 0, which means disabled. Higher values lead
%     to steeper rises, which is closer to the real signal but will again
%     lead to higher integration times.
%   smooth_step - When a step function is converted into a continuous
...
%     the result will be plotted after calculation. If set to 0, no plot
%     will be generated automatically. Default is 102.
%   plot_sim - Plot simulation result of operation after calculation. Must
%     be a positive integer. Defines the figure in which the result will be
%     plotted after calculation. If set to 0, no plot will be generated
...
% Interpolation of signal integral
default = 'spline';
validate = @(interp) any(validatestring(interp, ...
                         {'linear', 'pchip', 'spline'}));
prsr.addParameter('interint', default, validate)
...
% State Tol
prsr.addParameter('state_tol', 1e-2, @isnumeric);
