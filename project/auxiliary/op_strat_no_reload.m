function [p_base, p_peak, p_diff] = op_strat_no_reload(t, p_in, ...
                                                       e_base, e_peak, ...
                                                       soc_fcn, ...
                                                       storage_info, ...
                                                       signal_info, ...
...
p_diff = powers(:,3);
    ...
    function powers = standard_operation()
        ...
        % sign_eq = sign(peak_request) == sign(p_in);
        % sign_neq = sign(peak_request) ~= sign(p_in);

        % peak = sign_eq.*min(abs(p_in), abs(peak_request)).*sign(p_in) + ...
        %        sign_neq.*(-max(p_in - p_base_max, 0));
        % base = sign_eq.*(-max(p_in - peak_request, 0)) + ...
        %        sign_neq.*min(abs(p_in), abs(p_base_max)).*sign(p_in); 
        % diffp = - p_in - base - peak;

        % powers = [base, peak, diffp];
        peak_request = p_peak_max.*tanh(tanh_grad*(soc_peak - soc_aim));
        peak_request = sign(peak_request).* ...
                       min(abs(p_in),abs(peak_request)).* ...
                       (sign(peak_request) ~= sign(p_in));
